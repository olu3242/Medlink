-- Discovery outcome contract hardening, round 2: reserve_inventory never
-- verified that the inventory batch it was locking actually corresponded
-- to the medication access request's own requested_medicine_id.
-- match_inventory (202608170043) already enforces this exact check before
-- walking the MAR to 'matched' -- "batch.medicine_id is distinct from
-- mar.requested_medicine_id" raises 'Inventory is not an eligible
-- canonical match' -- but match_inventory only records that a match
-- happened via the MAR's own state, never which inventory_batch_id was
-- validated. reserve_inventory's target_inventory_batch_id is caller-
-- supplied and independent of whatever match_inventory actually checked,
-- so a patient (or any other caller with a legitimate 'matched' MAR) could
-- call POST /api/v1/reservations directly with an unrelated medicine's
-- inventory batch id -- including a pharmacist-governed generic-related
-- one that the discovery/UI layer deliberately marks reservationEligible:
-- false -- and reserve_inventory would lock it with no identity check at
-- all. This closes that gap with the exact same check match_inventory
-- already performs, not a new authority model. Ported forward onto
-- 202607290020's replay-validated body (the actual latest version),
-- adding one condition, not reverting its fix.
