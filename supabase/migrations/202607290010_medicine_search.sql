...
as $$
  with candidates as (
    select
      ...
      greatest(
        extensions.similarity(lower(medicine.brand_name), lower(search_term)),
        extensions.similarity(lower(coalesce(medicine.manufacturer_name, '')), lower(search_term))
      )::double precision as relevance,
      case
        when extensions.similarity(lower(medicine.brand_name), lower(search_term))
          >= extensions.similarity(lower(coalesce(medicine.manufacturer_name, '')), lower(search_term))
    ...
    where 'brand' = any(requested_types)
      ...
      and (
        medicine.brand_name operator(extensions.%) search_term
        or coalesce(medicine.manufacturer_name, '') operator(extensions.%) search_term
    ...
    select
      medicine.id,
      'generic'::text,
      extensions.similarity(lower(medicine.generic_name), lower(search_term))::double precision,
      'name'::text
    from public.medicines medicine
    where 'generic' = any(requested_types)
      ...
      and medicine.deleted_at is null
      and (
        medicine.generic_name operator(extensions.%) search_term
