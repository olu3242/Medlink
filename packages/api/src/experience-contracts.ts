...
export const runtimeServiceContracts: readonly RuntimeServiceContract[] = [
  ...
  { id: "payments", packageName: "@medlink/payments", status: "available", contracts: ["HostedPaymentProvider", "PaymentService", "PaymentRepository"], persistence: ["payments", "payment_attempts", "payment_events"] },
...
export const experienceOperationContracts: readonly ExperienceOperationContract[] = [
  ...
  { id: "patient.inventory.match", persona: "patient", method: "POST", path: "/api/v1/mar/:id/match", permission: "inventory:read", roles: ["patient"], workflow: "WF-008", services: ["runtime", "search", "inventory"], events: ["mar.transitioned.v1"], status: "available" },
  ...
  { id: "patient.payment.create", persona: "patient", method: "POST", path: "/api/v1/payments", permission: "payment:create", roles: ["patient"], workflow: "WF-010", services: ["runtime", "payments"], events: ["payment.attempt-created.v1"], status: "available" },
  { id: "patient.payment.list", persona: "patient", method: "GET", path: "/api/v1/payments", permission: "payment:read", roles: ["patient"], workflow: "WF-010", services: ["runtime", "payments"], events: [], status: "available" },
