...
export const governedAgentCatalog: readonly AgentIdentity[] = [
  ...
  {
    id: "clara",
    name: "Clara, Clinical Copilot",
    mission: "Narrate already-computed clinical data in plain language for a pharmacist's own review -- summarizes prescriptions, explains equivalency candidates packages/medicine already computed, and drafts (never sends) clarification questions. Never computes a clinical judgment of her own; every output is checked for directive language before use. Distinct from clinical-review-assistant (which records findings) -- Clara only narrates.",
    memoryBoundary: "none",
    status: "active",
    version: "0.1.0",
    owner: "clinical-ai-team",
    promptIds: [
      "clara_summarize_prescription",
      "clara_explain_equivalence_candidate",
      "clara_draft_clarification_request",
    ],
    capabilities: [
      {
        name: "summarize_prescription",
        description: "Plain-language summary of a prescription and its already-flagged clinical findings, for a pharmacist about to review it.",
        mutatesState: false,
        allowedRoles: ["pharmacist", "pharmacy_staff"],
        requiresHumanApproval: false,
        clinicalDecision: false,
      ...
      {
        name: "explain_equivalence_candidates",
        description: "Plain-language explanation of equivalency candidates CatalogEquivalencyService already computed. Never adds, removes, or ranks a candidate.",
        mutatesState: false,
        allowedRoles: ["pharmacist", "pharmacy_staff"],
        requiresHumanApproval: false,
        clinicalDecision: false,
      ...
      {
        name: "draft_clarification_request",
        description: "Drafts a clarification question for the pharmacist to review and send manually -- no delivery mechanism exists (G09 is unbuilt).",
        mutatesState: false,
        allowedRoles: ["pharmacist", "pharmacy_staff"],
        requiresHumanApproval: false,
        clinicalDecision: false,
