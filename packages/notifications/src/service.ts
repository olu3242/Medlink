...
// history) -- neither is reconstructable from {providerId} alone. Widened
// from an earlier (key)-only / (key, result)-only signature the one
// existing test still exercises with an in-memory fake; no other
// implementation existed yet to break.
