...
export async function signInWithPassword(formData: FormData) {
  ...
  if (!result.success) redirect(`/auth/sign-in?error=invalid_credentials&next=${encodeURIComponent(next)}`);
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.auth.signInWithPassword(result.data);
  if (error) redirect(`/auth/sign-in?error=${error.code === "email_not_confirmed" ? "email_unverified" : error.code?.includes("rate_limit") ? "rate_limited" : "invalid_credentials"}&next=${encodeURIComponent(next)}`);
  redirect(next);
}
...
export async function requestMagicLink(formData: FormData) {
  ...
  const next = safeNext(formData.get("next"));
  if (!email.success) redirect(`/auth/sign-in?error=invalid_email&next=${encodeURIComponent(next)}`);
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.auth.signInWithOtp({ email: email.data, options: {
    emailRedirectTo: `${appUrl()}/auth/callback?next=${encodeURIComponent(next)}`,
  } });
  if (error) redirect(`/auth/sign-in?error=${error.code === "over_email_send_rate_limit" ? "rate_limited" : "sign_in_failed"}&next=${encodeURIComponent(next)}`);
