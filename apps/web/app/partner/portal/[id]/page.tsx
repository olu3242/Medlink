...
export default async function PartnerApplicationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const {data:locations}=application.organization_id?await supabase.from("pharmacy_locations").select("id,name,license_number").eq("organization_id",application.organization_id).is("deleted_at",null).limit(1):{data:null};
  ...
  return <main className="partner-shell"><nav className="partner-nav"><Link className="brand" href="/"><span>M</span>MedLink</Link><Link href="/partner/portal">All applications</Link></nav>
    ...
    <section className="partner-section partner-detail-grid"><article className="partner-card"><h2>Readiness</h2>{assessment?<><strong className={assessment.ready?"ready":"blocked"}>{assessment.ready?"Ready for activation":"Blocked"}</strong><ul>{assessment.blockers.map((blocker)=><li key={blocker}>{blocker.replaceAll("_"," ")}</li>)}</ul></>:<p>No readiness assessment has been recorded.</p>}<PartnerActions applicationId={application.id} version={application.version} status={application.relationship_status} partnerType={application.partner_type} {...(location?{locationId:location.id}:{})} {...agreementProps}/></article>
