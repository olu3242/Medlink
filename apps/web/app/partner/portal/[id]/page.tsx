...
export default async function PartnerApplicationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="partner-shell"><nav className="partner-nav"><Link className="brand" href="/"><span>M</span>MedLink</Link><Link href="/partner/portal">All applications</Link></nav>
    ...
    <article className="partner-card"><h2>Canonical handoff</h2>{application.organization_id?<><p>Your approved relationship is linked to one MedLink organization.</p><code>{application.organization_id}</code>{["pharmacy","pharmacy_chain"].includes(application.partner_type)&&<a className="button" href="/pharmacy">Continue pharmacy onboarding</a>}</>:<p>An organization is linked only after a reviewer resolves the submitted identity.</p>}</article></section>
  </main>;
}
