...
import { getPlatformDashboard } from "../../../lib/admin/api";
...
export default async function ControlCenterPage() {
  try {
    ...
    return <>
      ...
      <section aria-labelledby="overview-heading"><h2 id="overview-heading">Authorized-scope overview</h2><p className="muted">{data.metricScope}</p><div className="metric-grid">{data.metrics.map((item) => <Link className="metric-card" href={item.href.replace(/^\/control-center/, "/admin")} key={item.id}><span>{item.label}</span><strong>{item.value.toLocaleString()}</strong><small data-status={item.status}>{item.status === "empty" ? "No onboarded data" : item.status}</small></Link>)}</div></section>
      <section aria-labelledby="attention-heading" className="card control-section"><h2 id="attention-heading">Needs attention</h2>{data.workQueue.length ? <ul className="work-queue">{data.workQueue.map((item) => <li key={item.title}><strong>{item.title}</strong><span>{item.reason}</span><Link href={item.href.replace(/^\/control-center/, "/admin")}>Review</Link></li>)}</ul> : <p className="empty-compact">No current actionable conditions.</p>}</section>
    ...
    return <section className="card error-state" role="alert"><h1>Control Center unavailable</h1><p>The authenticated dashboard could not be loaded. Confirm your platform-admin session and retry.</p><Link href="/admin">Retry</Link></section>;
  }
}
