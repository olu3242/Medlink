...
const pharmacyLocation = z.object({ name:z.string().min(2).max(160),licenseNumber:z.string().min(3).max(120),addressLine1:z.string().min(3).max(240),locality:z.string().min(2).max(120),countryCode:z.string().regex(/^[A-Za-z]{2}$/),latitude:z.number().min(-90).max(90),longitude:z.number().min(-180).max(180),idempotencyKey:z.string().min(8).optional() });
const pharmacistAssignment = z.object({ email:z.string().email().max(320),licenseNumber:z.string().min(3).max(120),issuingAuthority:z.string().min(2).max(160),licenseExpiresOn:z.string().date(),reason:z.string().min(10).max(2000),idempotencyKey:z.string().min(8).optional() });
...
export async function POST(request: Request, context: Context) {
  ...
  } else if (params.action === "location") {
    const body=pharmacyLocation.safeParse(raw); if(!body.success) return partnerProblem(400,"invalid_pharmacy_location","Complete licensed pharmacy location details are required");
    result=await partnerRpc(database,"create_partner_pharmacy_location",{target_application_id:id.data,target_name:body.data.name,target_license_number:body.data.licenseNumber,target_address_line_1:body.data.addressLine1,target_locality:body.data.locality,target_country_code:body.data.countryCode,target_latitude:body.data.latitude,target_longitude:body.data.longitude,target_idempotency_key:idempotencyKey(request,body.data.idempotencyKey)});
  } else if (params.action === "pharmacist") {
    const body=pharmacistAssignment.safeParse(raw); if(!body.success) return partnerProblem(400,"invalid_pharmacist_assignment","Complete verified pharmacist credential evidence is required");
    result=await partnerRpc(database,"assign_partner_pharmacist",{target_application_id:id.data,target_email:body.data.email,target_license_number:body.data.licenseNumber,target_issuing_authority:body.data.issuingAuthority,target_license_expires_on:body.data.licenseExpiresOn,target_reason:body.data.reason,target_idempotency_key:idempotencyKey(request,body.data.idempotencyKey)});
