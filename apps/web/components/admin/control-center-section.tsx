...
import { getDashboardSection } from "../../lib/admin/api";
...
export async function ControlCenterSection({ section, title, description, searchParams }: Props) {
  ...
  try {
    ...
    return <>
      ...
      {data.metrics ? <section className="control-section"><h2>Health metrics</h2><div className="metric-grid">{data.metrics.map((item) => <Link className="metric-card" href={item.href.replace(/^\/control-center/, "/admin")} key={item.id}><span>{item.label}</span><strong>{item.value.toLocaleString()}</strong><small data-status={item.status}>{item.status}</small></Link>)}</div></section> : null}
    ...
    return <section className="card error-state" role="alert"><h1>{title} unavailable</h1><p>The request failed or the selected filter is outside your authorized scope.</p><Link href={`/admin/${section}`}>Retry</Link></section>;
  }
}
