...
const decisionSchema = z.object({
  ...
  rationale: z.string().min(3).max(4000).optional(),
  recommendation: z.string().min(3).max(4000).optional(),
...
}).refine((value) => Boolean(value.rationale ?? value.recommendation), {
  message: "A pharmacist rationale is required",
});
