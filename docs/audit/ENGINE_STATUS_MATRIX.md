...
| Conversation Engine | 3 | Partial | Conditional | `packages/conversation` (models/ports/`ConversationEngine`/default keyword intent classifier) and migration `202607290012` (`conversations`/`conversation_messages`/append-only `conversation_events`, RLS) exist with unit and static RLS/migration tests; no route, no Supabase-backed port implementation, no WhatsApp adapter yet | Implement the WhatsApp adapter and Supabase-backed ports (RC1_BACKLOG item 15) |
