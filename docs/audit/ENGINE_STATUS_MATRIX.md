...
| Conversation Engine | 3 | Partial | Conditional | `packages/conversation` (models/ports/`ConversationEngine`/default keyword intent classifier) and migration `202607290012` (`conversations`/`conversation_messages`/append-only `conversation_events`, RLS) exist with unit and static RLS/migration tests; no route or Supabase-backed port implementation yet, blocked on the `RuntimeContext.userId` ADR below | Accept an ADR for webhook identity in `RuntimeContext`, then wire a route |
